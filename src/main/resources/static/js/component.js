...
Vue.component("tt-menu-root", {
    ...
    render: function (createElement) {
        ...
        //权限
        if (!Shiro.requiresRoles(self.data.role, self.data.logical) || !Shiro.requiresPermissions(self.data.permission, self.data.logical)) {
            return null;
        }

        if (self.data.code == "Home") {
            let elements = [];
            if (!self.isEmpty(self.data.icon)) {
                elements.push(createElement("i", { class: ["fa", self.data.icon] }));
            }
            elements.push(createElement("span", { class: ["nav-label"] }, self.data.name));
            if (self.isEmpty(self.data.list)) {
        ...
        for (let index = 0; index < sessionPermission.length; index++) {
            ...
            if (typeof (self.data.permission) === "string") {
                if (element.indexOf(self.data.permission) > -1) {
                    let elements = [];
                    if (!self.isEmpty(self.data.icon)) {
                        elements.push(createElement("i", { class: ["fa", self.data.icon] }));
                    }
...
Vue.component('tt-table', {
    props: ['value', 'data', 'selection', 'selectOne'],
    template: '<table class="table table-striped table-hover">' +
        ...
        '<td v-if="selection">' +
        '<div class="checkbox checkbox-table tt-table-checkbox">' +
        '<input v-model="checkedData" v-bind:value="item" v-on:click="updateSelect(item.id)" type="checkbox">' +
        '<label></label>' +
        '</div>' +
        '</td>' +
        //表格主体默认样式
...
Vue.component('tt-pagination', {
    ...
    methods: {
        ...
        goToInputIndex: function () {
            if (this.inputIndex === this.innerIndex) {
...
Vue.component('tt-simple-input', {
    props: ['value', 'name', 'label', 'type', 'row', 'placeholder', 'required', 'minlength', 'maxlength', 'disabled', 'readonly'],
    render: function (createElement) {
        let self = this;
...
Vue.component("tt-simple-tree-children", {
    props: ['data', 'value', 'required', 'func'],
    template: '<div v-if="data.length !== 0">' +
        '<select v-model="object1" class="form-control" :required="required">' +
        '<option :value="null">---- 请选择 ----</option>' +
        '<option v-for="item in data" :value="item.key">{{ item.value }}</option>' +
        '</select>' +
        '<tt-simple-tree-children v-if="hasTree" :data="func(object1)" :func="func" :required="required" :value="object2" @input="updateValue"></tt-simple-tree-children>' +
        '</div>',
    data: function () {
        return {
            object1: this.value,
